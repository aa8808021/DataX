...
public class ColumnValueTransformHelper {
    public static Column otsPrimaryKeyValueToDataxColumn(PrimaryKeyValue pkValue) {
        switch (pkValue.getType()) {
            case STRING:
                return new StringColumn(pkValue.asString());
            case INTEGER:
                return new LongColumn(pkValue.asLong());
            case BINARY:
                return new BytesColumn(pkValue.asBinary());
            default:
    ...
    public static Column otsColumnValueToDataxColumn(ColumnValue columnValue) {
        switch (columnValue.getType()) {
            case STRING:
                return new StringColumn(columnValue.asString());
            case INTEGER:
                return new LongColumn(columnValue.asLong());
            case BINARY:
                return new BytesColumn(columnValue.asBinary());
            case BOOLEAN:
                return new BoolColumn(columnValue.asBoolean());
            case DOUBLE:
                return new DoubleColumn(columnValue.asDouble());
            default:
